...
Evidence is a hash-chained, append-only decision ledger for Salesforce.
...
| Control | Count |
|---|---|
| Production Apex classes | 19 |
| Classes declared `with sharing` | 18 |
| Classes declared `without sharing` | 0 |
| Production SOQL queries | 32 |
| Queries carrying an explicit enforcement clause | 29 |
| Queries deliberately running in system mode | 3 |
| DML statements using `as user` | 11 |
| Apex test methods | 89 |

3 production queries do not carry an enforcement clause. Every one is
listed here with the reason, and there are no others:

| Location | Queries | Why system mode is correct there |
|---|---|---|
| `EvidenceSharingService` | 2 | Reads sharing metadata in order to apply record shares. It must see the sharing configuration itself, which is not user-visible data. |
| `EvidenceLedgerStore` | 1 | Reads the ledger's own hash-chain head in order to append. Integrity of the chain does not depend on who is writing. |
...
- It does not permit edits to a recorded entry. The ledger is append-only by design.
- It does not call any external service.
- It does not use the hash chain as a substitute for access control. Integrity and confidentiality are separate properties.
...
- Grant access through the permission set
  (6 shipped in this repository), not by widening profiles.
