 ...
 *
 * Gradient-filled series over a horizontal grid, with a pointer-tracked
 * tooltip that snaps to the nearest sample and a crosshair down to the axis.
 *
 * Gradient ids come from useId. Two instances of this chart on one page would
 * otherwise emit duplicate ids, and SVG resolves url(#id) against the first
 * match in the document - so the second chart would silently paint with the
 * first one's fill.
 */
...
export function AreaTrendChart({
  ...
  const { max, ticks } = React.useMemo(() => {
    const peak = Math.max(1, ...series.flatMap((s) => s.values), reference?.value ?? 0);
  ...
  const y = React.useMemo(() => linearScale([0, max], [plot.bottom, plot.top]), [max, plot.bottom, plot.top]);
        ...
        {reference && (
          <g>
            <line
              ...
              stroke={reference.color ?? INK.warning}
              strokeWidth={1.25}
              strokeDasharray="5 4"
              opacity={0.75}
            />
            <text
              ...
              textAnchor="end"
              style={{ fontFamily: FONT, fontSize: 10.5, fill: reference.color ?? INK.warning, fontWeight: 600 }}
            >
              {reference.label}
            </text>
          </g>
          ...
          <g key={s.key}>
            ...
            <path
              ...
              style={
                reduceMotion
                  ? undefined
                  : { animation: `evidence-draw 700ms ease-out both`, strokeDasharray: 2400, strokeDashoffset: 0 }
              }
            />
          </g>
        ))}
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 10,
        boxShadow: "0 8px 24px -8px rgba(28,22,38,0.24)",
        padding: "9px 11px",
        pointerEvents: "none",
        minWidth: 132,
        zIndex: 2,
    ...
    >
      <div style={{ fontFamily: DISPLAY, fontSize: 11.5, fontWeight: 600, color: INK.text, marginBottom: 6 }}>
        {label}
      </div>
      {series.map((s) => (
        <div key={s.key} style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 3 }}>
          ...
          <span style={{ fontFamily: FONT, fontSize: 11.5, color: INK.weak, flex: 1 }}>{s.label}</span>
          <span style={{ fontFamily: MONO, fontSize: 11.5, fontWeight: 600, color: INK.text }}>{s.text}</span>
        </div>
      ))}
    </div>
  );
