 ...
 *
 * Segments are stroked arcs on one circle rather than filled wedge paths: a
 * stroked circle takes a dash array, so each segment is two numbers and an
 * offset, and the rounded stroke caps come free.
 */
...
export function DonutChart({
  ...
  const total = React.useMemo(() => slices.reduce((sum, s) => sum + Math.max(0, s.value), 0), [slices]);
  ...
  return (
    <div style={{ position: "relative", width: size, height: size, margin: "0 auto" }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} role="img" aria-label={`${centerLabel}: ${centerValue}`}>
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke={INK.page} strokeWidth={thickness} />
        {arcs.map((arc) => {
      ...
      >
        <div style={{ fontFamily: DISPLAY, fontSize: 26, fontWeight: 600, color: INK.text, letterSpacing: "-0.01em" }}>
          {centerValue}
        </div>
        <div style={{ fontFamily: FONT, fontSize: 11, color: INK.weakest, marginTop: 2, maxWidth: size - 60 }}>
          {centerLabel}
        </div>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <span style={{ minWidth: 0, flex: 1 }}>
        ...
        {meta && (
          <span style={{ display: "block", fontFamily: FONT, fontSize: 11, color: INK.weakest, marginTop: 1 }}>
            {meta}
          </span>
        )}
      </span>
      <span style={{ fontFamily: MONO, fontSize: 12, fontWeight: 600, color: INK.text, flexShrink: 0 }}>
        {value}
      </span>
    </div>
  );
